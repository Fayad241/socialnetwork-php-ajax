<?php 
    ...
    while($user = $sql->fetch(PDO::FETCH_ASSOC)) {
        ...
        $output .= '
            <div class="flex justify-between my-4 cursor-pointer user-item" data-user-id="'.$user['unique-id'].'">
                <div class="flex gap-4">
                    <div class="relative">
                        <img class="w-13 h-13 rounded-full object-cover" src="profile-pic/'. htmlspecialchars($user['profile-pic']) .'" alt="">
                    ...
                    <div>
                        ...
                        <p class="last-message text-gray-600">'. $you .' '. htmlspecialchars($msg) .'</p>
                    </div>
                </div>

                <div class="flex gap-2 flex-col items-center">
                    <p class="last-message-date text-gray-600">'.$date_display.'</p>';

                    if($showNotification){
                        $output .= '<div class="user-notif bg-blue-500 w-3 h-3 rounded-full animate-pulse"></div>';
                    }
